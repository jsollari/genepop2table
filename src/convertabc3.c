...
int main(int argc, char *argv[]){
	...
	if(argc != 3)
		printerr("needs sample file, output filename (no extension)");

	inp = fopen(argv[1],"r");
	if(inp == NULL)
		printerr("cannot open sample file");
	
	outsize = strlen(argv[2]) + 5;
	...
	if(loctype){
		...
		while(searchPop){
			fgets(aux,MAXCHAR,inp);
			if(feof(inp))
				printerr("file ended before the display of data");
			j=0;
			...
			if(searchPop) //JL-22.10.09: corrected
				i++;
		}
		nloc+=i;
	}
		...
		for(csamp=0; csamp<lsamp[cpop] ; csamp++){
			...
			for(cloc=0; cloc<nloc; cloc++){
				...
				if(strlen(info)==2){
					...
					printerr("incorrect number of digits in locus");
